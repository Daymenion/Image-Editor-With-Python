...
class EdgeDetectionTopLevel(Toplevel):
    def __init__(self, master=None):
        ...
        self.start_detection = Button(master=self, text="Face-Mask Detection")
        ...
        self.thresholdParam_scale = Scale(self, from_=0, to=255, length=300, resolution=0.1, activebackground="gray",
        ...
        self.thresholdParam2_scale = Scale(self, from_=0, to=255, length=300, resolution=0.1, activebackground="gray",
                                           cursor="arrow", orient=HORIZONTAL)

        self.areaParam_label = Label(self, text="Area Parameter")
        self.areaParam_scale = Scale(self, from_=300, to=10000, length=300, resolution=1, activebackground="gray",
                                     cursor="arrow", orient=HORIZONTAL)

        self.start_button.bind("<ButtonRelease>", self.start_button_released)
        self.start_detection.bind("<ButtonRelease>", self.start_detection_released)
        ...
        self.start_detection.pack(side=TOP)
        self.finish_button.pack(side=LEFT)

    def start_button_released(self, event):
        self.video_capture()

    def start_detection_released(self, event):
        self.mask_detection()
    ...
    def mask_detection(self):
        ...
        # Opencv versiyonu "pip install opencv-contrib-pythondur"
        net = cv2.dnn.readNet("dnn_model/yolov4-tiny-custom_best.weights", "dnn_model/yolov4-tiny-custom.cfg")
        ...
        while True:
            ...
            # Object Detection
            (class_ids, scores, bboxes) = model.detect(frame, confThreshold=0.6, nmsThreshold=.4)
            for class_id, score, bbox in zip(class_ids, scores, bboxes):
                ...
                if class_name in classes:
                    cv2.putText(frame, class_name + '  ' + str(format(score, '0.2')), (x, y - 10),
